...
export type Session={userId:string;workspaceId:string;name:string;email:string;sessionVersion:number};
...
export async function getSession():Promise<Session|null>{
  ...
  try{
    ...
    const session:Session={userId:String(payload.userId),workspaceId:String(payload.workspaceId),name:String(payload.name),email:String(payload.email),sessionVersion:Number(payload.sessionVersion??0)};
    const [user]=await getDb().select({sessionVersion:users.sessionVersion}).from(users).where(eq(users.id,session.userId)).limit(1);
...
export async function requireApiSession(){const session=await getSession();if(!session)return null;return session}
